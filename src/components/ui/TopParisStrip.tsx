      ...
      <div className="mt-5 grid gap-4 xl:grid-cols-3">
        {list.map((item) => (
          <button
            ...
            className="group flex h-full flex-col rounded-[1.6rem] border border-[var(--pmu-border)] bg-[var(--pmu-surface-2)] p-5 text-left transition duration-300 hover:-translate-y-0.5 hover:border-[color-mix(in_srgb,var(--pmu-primary)_40%,transparent)] hover:shadow-[var(--pmu-glow)]"
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[color-mix(in_srgb,var(--pmu-primary)_35%,transparent)] bg-[var(--pmu-primary-soft)] text-lg font-black text-[var(--pmu-primary)]">
                #{item.rank}
              </div>
              <span className="rounded-full border border-[var(--pmu-border-strong)] px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-[var(--pmu-text-muted)]">
                {item.sourceLabel}
              </span>
            </div>
