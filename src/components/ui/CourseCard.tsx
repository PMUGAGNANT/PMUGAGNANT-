...
export function CourseCard({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group app-card flex h-full w-full flex-col gap-5 p-5 text-left transition duration-200 hover:-translate-y-0.5 hover:border-[rgba(0,255,136,0.36)] hover:shadow-[0_28px_56px_rgba(0,0,0,0.36)]"
    >
      <div className="grid gap-4 xl:grid-cols-[0.78fr,1.2fr,0.95fr] xl:items-start">
        <div className="space-y-2">
          <p className="text-3xl font-black leading-none tracking-tight text-white">{timeLabel}</p>
        ...
        <div className="space-y-3">
          <div>
            <h3 className="text-2xl font-black leading-tight tracking-tight text-white">{raceTitle}</h3>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-sm font-semibold text-white">{horseLabel}</span>
          </div>
        </div>
