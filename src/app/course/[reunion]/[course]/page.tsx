...
import type { PredictionStageSnapshotRow, RaceAnalysis, RaceSummary, ScoredParticipant } from "@/lib/types";
...
interface RaceApiResponse {
  ...
  timelineSnapshots?: PredictionStageSnapshotRow[];
  paywall?: {
    required: boolean;
    preview?: {
      lisibilite: string;
...
interface SignalTimelineStep {
  ...
  meta?: string[];
  stage?: PredictionStageSnapshotRow["stage"] | "FALLBACK";
}
...
function getMarketTopRunners(ranking: ScoredParticipant[]) {
  return [...ranking]
    .filter((runner) => runner.cote != null && Number.isFinite(runner.cote) && (runner.cote ?? 0) > 0)
    .sort((left, right) => (left.cote ?? 99) - (right.cote ?? 99))
    .slice(0, 3);
}

function buildFalseFavoriteInsight(
  technicalFavorite: ScoredParticipant | null,
  marketFavorite: ScoredParticipant | null
) {
  if (!technicalFavorite || !marketFavorite || technicalFavorite.numPmu === marketFavorite.numPmu) {
    return null;
  }

  const marketConfidence = marketFavorite.prediction?.confiance ?? 0;
  const engineConfidence = technicalFavorite.prediction?.confiance ?? 0;
  const confidenceGap = round1(engineConfidence - marketConfidence);

  if (confidenceGap < 0.8) {
    return null;
  }

  return {
    title: "Faux favori potentiel",
    body: `Le marche pousse N${marketFavorite.numPmu} ${marketFavorite.nom}, mais le moteur prefere N${technicalFavorite.numPmu} ${technicalFavorite.nom} avec ${confidenceGap} pt(s) d'avance en confiance.`,
  };
}

function buildSignalTimeline(
  data: RaceApiResponse | null,
  analysis: RaceAnalysis | null,
  simpleTicket: ScoredParticipant | null
): SignalTimelineStep[] {
  if (!data) return [];

  const persistedSteps = asArray<PredictionStageSnapshotRow>(data.timelineSnapshots);
  if (persistedSteps.length > 0) {
    const stageLabels: Record<PredictionStageSnapshotRow["stage"], { id: string; title: string }> = {
      MATIN: { id: "morning", title: "Scan matin" },
      T10: { id: "t10", title: "Signal T-10" },
      RESULTAT: { id: "result", title: "Resultat" },
    };

    return persistedSteps.map((snapshot, index) => {
      const label = stageLabels[snapshot.stage];
      const selection =
        snapshot.selection_num && snapshot.selection_nom
          ? `N${snapshot.selection_num} ${snapshot.selection_nom}`
          : "Pas de selection nette";
      const decision = snapshot.selection_decision ? snapshot.selection_decision.toLowerCase() : "en observation";
      const confidence =
        snapshot.selection_confiance != null ? `Confiance ${round1(snapshot.selection_confiance)}/10` : "Confiance a confirmer";
      const notes = asArray<string>(snapshot.notes).slice(0, 2).join(" · ");

      return {
        id: label.id,
        title: label.title,
        status: data.isFinished ? "done" : index === persistedSteps.length - 1 ? "active" : "done",
        kicker:
          snapshot.stage === "MATIN"
            ? "Premiere lecture"
            : snapshot.stage === "T10"
              ? "Ajustement pre-depart"
              : "Debrief officiel",
        emphasis: selection,
        meta: [
          snapshot.selection_decision ? `Decision ${snapshot.selection_decision.toLowerCase()}` : null,
          snapshot.selection_pari ? formatTicketType(snapshot.selection_pari) : null,
          snapshot.selection_cote != null ? `Cote ${formatOdds(snapshot.selection_cote)}` : null,
          snapshot.market_favorite_nom ? `Marche ${snapshot.market_favorite_nom}` : null,
        ].filter((value): value is string => Boolean(value)),
        stage: snapshot.stage,
        detail: [
          `${selection} · ${decision}`,
          snapshot.lisibilite ? `Lisibilite ${snapshot.lisibilite}` : null,
          confidence,
          notes || null,
        ]
          .filter(Boolean)
  ...
  const ticketLabel = simpleTicket ? `N${simpleTicket.numPmu} ${simpleTicket.nom}` : "Ticket en attente";
  const lisibilite = analysis?.prediction.lisibilite ?? "N/A";
  const confidence = analysis?.scoreConfiance?.score != null ? `${analysis.scoreConfiance.score}/10` : "N/A";
  const m = data.minutesUntilStart;
  ...
  return [
    {
      id: "morning",
      title: "Scan matin",
      status: analysis ? "done" : "upcoming",
      kicker: "Premiere lecture",
      emphasis: ticketLabel,
      meta: analysis ? [`Lisibilite ${lisibilite}`, `Confiance ${confidence}`] : [],
      stage: "FALLBACK",
      detail: analysis
        ? `Le moteur a identifié ${ticketLabel} comme repere initial.`
        : "Le scan initial n'a pas encore remonté de lecture exploitable.",
    },
    {
      id: "one-hour",
      title: "Fenetre 1h",
      status: isFinished || m <= 60 ? (m > 30 && !isFinished ? "active" : "done") : "upcoming",
      kicker: "Lecture intermediaire",
      emphasis: analysis ? `${lisibilite} · ${confidence}` : "Lecture en attente",
      meta: simpleTicket ? [formatTicketType(simpleTicket.prediction.typePariConseille)] : [],
      stage: "FALLBACK",
      detail: `Lisibilite ${lisibilite} avec une confiance moteur de ${confidence}.`,
    },
    {
      id: "thirty-minutes",
      title: "Signal 30 min",
      status: isFinished || m <= 30 ? (!isFinished ? "active" : "done") : "upcoming",
      kicker: "Ticket final",
      emphasis: simpleTicket ? ticketLabel : "Ticket en attente",
      meta: simpleTicket ? [formatTicketType(simpleTicket.prediction.typePariConseille)] : [],
      stage: "FALLBACK",
      detail: simpleTicket
        ? `${ticketLabel} ressort en ${formatTicketType(simpleTicket.prediction.typePariConseille)} juste avant le depart.`
        : "Le ticket final s'ouvre a l'approche du depart.",
    },
    {
      id: "result",
      title: "Resultat",
      status: isFinished ? "done" : "upcoming",
      kicker: "Debrief officiel",
      emphasis: isFinished ? "Arrivee disponible" : "En attente d'arrivee",
      meta: [],
      stage: "FALLBACK",
      detail: isFinished
        ? "La course est bouclee: le debrief officiel est disponible plus bas."
        : "Le debrief resultat apparaitra apres l'arrivee officielle.",
    },
  ];
}
...
export default function CourseDetailPage({ params }: { params: Promise<{ reunion: string; course: string }> }) {
  ...
  const marketTopRunners = useMemo(() => getMarketTopRunners(rankingAll), [rankingAll]);
  const marketFavorite = marketTopRunners[0] ?? null;
  const falseFavoriteInsight = useMemo(
    () => buildFalseFavoriteInsight(technicalFavorite, marketFavorite),
    [technicalFavorite, marketFavorite]
  );
  const signalTimeline = useMemo(
    () => buildSignalTimeline(data, analysis, simpleTicket),
    [data, analysis, simpleTicket]
  );
      ...
      <div style={{ padding: "20px 16px", display: "grid", gap: 16 }} className="fade-up">
        ...
        {analysis && simpleTicket && (
          <>
            ...
            >
              <SectionCard title="Timeline du signal" kicker="Matin -> 1h -> 30 min -> resultat" accent={`${G}33`}>
                <div style={{ display: "grid", gap: 12 }}>
                  {signalTimeline.map((step, index) => {
                    const tone =
                      step.status === "done"
                        ? { bg: G_DIM, color: G, border: `${G}33` }
                        : step.status === "active"
                          ? { bg: BLUE_DIM, color: BLUE, border: `${BLUE}33` }
                          : { bg: CARD2, color: MUTED, border: BORDER };
                    const stageAccent =
                      step.stage === "RESULTAT"
                        ? GOLD
                        : step.stage === "T10"
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          boxShadow: step.status === "active" ? `0 18px 34px ${tone.border}` : "none",
                        }}
                      >
                        <div
                          style={{
                        ...
                        <div>
                          ...
                          >
                            <div>
                              {step.kicker ? (
                                <div style={{ fontSize: 10, color: stageAccent, textTransform: "uppercase", letterSpacing: "0.12em", marginBottom: 4, fontWeight: 800 }}>
                                  {step.kicker}
                                </div>
                              ) : null}
                            ...
                            <Tag color={tone.color} bg={CARD}>
                              {step.status === "done" ? "fait" : step.status === "active" ? "actif" : "a venir"}
                            </Tag>
                          </div>
                          {step.emphasis ? (
                            <div style={{ fontFamily: "var(--font-display)", fontSize: 18, fontWeight: 800, color: WHITE, lineHeight: 1.2, marginBottom: 6 }}>
                              {step.emphasis}
                            </div>
                          ) : null}
                          {step.meta && step.meta.length > 0 ? (
                            <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
                              {step.meta.map((item) => (
                                <Tag key={`${step.id}-${item}`} color={stageAccent} bg={CARD}>
                                  {item}
                                </Tag>
                              ))}
                            </div>
              ...
              <SectionCard title="Moteur vs marche" kicker="Faux favori et consensus" accent={`${BLUE}33`}>
                {falseFavoriteInsight ? (
                  <div
                    style={{
                      padding: 14,
                      borderRadius: 10,
                  ...
                  >
                    <div style={{ fontSize: 11, color: RED, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 8, fontWeight: 800 }}>
                      {falseFavoriteInsight.title}
                    </div>
                    <div style={{ fontSize: 13, color: WHITE, lineHeight: 1.6 }}>
                      {falseFavoriteInsight.body}
                    </div>
                  </div>
                ) : (
                  <div
                  ...
                  >
                    Le moteur et le marche restent globalement alignes sur le repere principal de la course.
                  </div>
                )}

                <div style={{ display: "grid", gap: 10 }}>
                  <div style={{ padding: 12, borderRadius: 10, background: CARD2, border: `1px solid ${BORDER}` }}>
                    <div style={{ fontSize: 10, color: MUTED, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 8 }}>
                      Top 3 moteur
                    </div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                      {top5Runners.slice(0, 3).map((runner) => (
                        <Tag key={`engine-${runner.numPmu}`} color={G} bg={G_DIM}>
                  ...
                  <div style={{ padding: 12, borderRadius: 10, background: CARD2, border: `1px solid ${BORDER}` }}>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                      {marketTopRunners.length > 0 ? marketTopRunners.map((runner) => (
                        <Tag key={`market-${runner.numPmu}`} color={BLUE} bg={BLUE_DIM}>
                          N{runner.numPmu} {runner.nom} @ {formatOdds(runner.cote)}
                        </Tag>
                      )) : <div style={{ fontSize: 13, color: MUTED }}>Consensus PMU indisponible pour cette course.</div>}
                    </div>
                  </div>
                </div>
              </SectionCard>
