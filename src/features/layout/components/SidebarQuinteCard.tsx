...
type SidebarQuinteState =
  ...
  | { status: "ready"; race: RaceSummary; score: RaceScore | null };

function normalizeDateParam(value: string | null) {
  return value && /^\d{8}$/.test(value) ? value : getTodayDateStr();
}

function toApiRaceScoreLite(score: RaceScore | null): ApiRaceScoreLite | undefined {
  if (!score) {
    return undefined;
  }
...
export function SidebarQuinteCard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const racesPayload = await fetchRacesForDate<RacesResponse>(selectedDate);

        if (!racesPayload.success) {
        ...
        let score: RaceScore | null = null;

        if (minutesUntilStart <= 120) {
          const scoresPayload = await fetchRaceScoresForDate<ScoresResponse>(selectedDate);

          if (scoresPayload?.success) {
            const scores = normalizeRaceScoresPayload<RaceScore>(
              scoresPayload.scores ?? null,
              selectedDate
            );
            score =
              scores.find(
