...
export function buildPredictionStageSnapshot(
  ...
  const marketFavorite =
    rows
      ...
      .sort(
        (left, right) =>
          (left.cote_depart ?? left.cote_matin ?? 999) -
          (right.cote_depart ?? right.cote_matin ?? 999)
      )[0] ?? null;
    ...
    return {
      ...
      payload: {
        ...
        expertPrediction: {
          ...
          scoreBlended: runner.prediction.scoreBlended ?? runner.prediction.scoreCheval,
          qualite: runner.prediction.qualite,
          confiance: runner.prediction.confiance,
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      blend_payload: {
        ...
        scoreBlended: row.score_blended ?? row.score_cheval,
