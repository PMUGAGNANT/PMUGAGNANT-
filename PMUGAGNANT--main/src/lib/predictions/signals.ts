...
const ELITE_PERFORMERS_TTL_MS = 60 * 60 * 1000;

interface ElitePerformerMaps {
  jockeyFlat: Record<string, number>;
  jockeyTrot: Record<string, number>;
  trainers: Record<string, number>;
}

const EMPTY_ELITE_PERFORMERS: ElitePerformerMaps = {
  jockeyFlat: {},
  jockeyTrot: {},
  trainers: {},
};

let elitePerformersCache: {
  data: ElitePerformerMaps;
  expiresAt: number;
} | null = null;

function normalizeElitePerformerName(value?: string | null) {
...
function setEliteScore(
  maps: ElitePerformerMaps,
  type: ElitePerformerType,
  name: string,
  score: number
) {
  const normalized = normalizeElitePerformerName(name);
  if (!normalized || !Number.isFinite(score)) return;
  const safeScore = clamp(Math.round(score), 1, 10);

  if (type === "JOCKEY_FLAT") {
    maps.jockeyFlat[normalized] = safeScore;
  } else if (type === "JOCKEY_TROT") {
    maps.jockeyTrot[normalized] = safeScore;
  } else {
    maps.trainers[normalized] = safeScore;
  }
}

function rowsToEliteMaps(rows: ElitePerformerRow[]): ElitePerformerMaps {
  const maps: ElitePerformerMaps = {
    jockeyFlat: {},
    jockeyTrot: {},
    trainers: {},
  };
  ...
  for (const row of rows) {
    ...
    setEliteScore(maps, row.type, row.nom_normalise, row.score);
  }

  return maps;
}

export function clearElitePerformersCache() {
  elitePerformersCache = null;
}

export async function loadElitePerformers(force = false): Promise<ElitePerformerMaps> {
  const now = Date.now();
  if (!force && elitePerformersCache && elitePerformersCache.expiresAt > now) {
    return elitePerformersCache.data;
  }

  const admin = getSupabaseAdminClient();
  if (!admin) {
    return elitePerformersCache?.data ?? EMPTY_ELITE_PERFORMERS;
  }

  const { data, error } = await admin
    .from("elite_performers")
    .select("id,type,nom_normalise,score,actif,updated_at")
    .eq("actif", true)
    .order("type", { ascending: true })
    .order("score", { ascending: false });

  if (error) {
    logger.warn("elite_performers.load_failed", { error: error.message });
    return elitePerformersCache?.data ?? EMPTY_ELITE_PERFORMERS;
  }

  const maps = rowsToEliteMaps((data ?? []) as ElitePerformerRow[]);
  elitePerformersCache = {
    data: maps,
    expiresAt: now + ELITE_PERFORMERS_TTL_MS,
  };

  return maps;
}

function getCachedElitePerformers() {
  return elitePerformersCache?.data ?? EMPTY_ELITE_PERFORMERS;
}

function getEliteScore(target: string, eliteMap: Record<string, number>) {
  const normalized = normalizeElitePerformerName(target);
...
function getHumanSignal(participant: Participant, race: RaceSummary) {
  ...
  const elite = getCachedElitePerformers();
  return race.estPlat
    ? getEliteScore(human, elite.jockeyFlat)
    : getEliteScore(human, elite.jockeyTrot);
}

function getTrainerSignal(participant: Participant) {
  return getEliteScore(participant.entraineur, getCachedElitePerformers().trainers);
}

function getJockeyFormSignal(participant: Participant, race: RaceSummary) {
  const elite = getCachedElitePerformers();
  const humanBase = race.estPlat
    ? getEliteScore(participant.jockey || participant.driver, elite.jockeyFlat)
    : getEliteScore(participant.driver || participant.jockey, elite.jockeyTrot);
...
export function determineRaceReadabilityScore(
  ...
  if (top3.length > 0) {
    const getReadableScore = (runner: ScoredParticipant) =>
      runner.prediction.scoreBlended ?? runner.prediction.scoreCheval;
    const gap12 =
      top3.length > 1 ? getReadableScore(top3[0]) - getReadableScore(top3[1]) : getReadableScore(top3[0]);
    const gap13 =
      top3.length > 2 ? getReadableScore(top3[0]) - getReadableScore(top3[2]) : gap12;
