...
import { blendScore } from "@/lib/engine/blend";
...
export function analyzeRaceWithParameters(
  ...
  const preRanked = participants
    ...
    .map((participant) => {
      ...
      const scoreBlended = blendScore(scoreCheval, segmentKey, parameters);

      return {
        ...participant,
        score: round1(scoreBlended),
        scoreAlgo: round1(scoreBlended),
        scoreBlended: round1(scoreBlended),
    ...
    .sort(
      (left, right) =>
        (right.prediction.scoreBlended ?? right.prediction.scoreCheval) -
        (left.prediction.scoreBlended ?? left.prediction.scoreCheval)
    );
  ...
  const rawStrengths = preRanked.map((runner) => {
    const blendedScore = runner.prediction.scoreBlended ?? runner.prediction.scoreCheval;
    return (
      Math.pow(Math.max(blendedScore, 1), 1.18) *
  ...
  const ranked = preRanked.map((runner, index) => {
    ...
    const blendedScore = runner.prediction.scoreBlended ?? runner.prediction.scoreCheval;
    const scoreFinalPari = round2(blendedScore * coefficientLisibilite);
    ...
    const decisionState = determineHorseDecision(
      {
        ...runner,
        scoreCheval: blendedScore,
        qualite: Math.round(blendedScore),
        confiance,
        scoreFinalPari,
        top3Potential,
        top5Potential,
    ...
    return {
      ...runner,
      score: round1(blendedScore),
      scoreAlgo: round1(scoreFinalPari),
      scoreBlended: round1(blendedScore),
      prediction: {
  ...
  const pepiteDuJour = // v9.3
    ranked
      ...
      .sort((a, b) => {
        const ratioA =
          (a.prediction.scoreBlended ?? a.prediction.scoreCheval) / Math.max(a.cote ?? 1, 1);
        const ratioB =
          (b.prediction.scoreBlended ?? b.prediction.scoreCheval) / Math.max(b.cote ?? 1, 1);
