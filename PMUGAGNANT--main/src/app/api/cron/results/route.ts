...
import { runResultSync } from "@/lib/prediction-pipeline";
...
export async function GET(request: NextRequest) {
  ...
  try {
    const summary = await runResultSync(date, {
      reunion,
      course,
    });
    return NextResponse.json(summary);
  } catch (error) {
    return serverError("Results cron failed", error, { date, reunion, course });
  }
}
